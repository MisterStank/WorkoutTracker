...
func (r *WorkoutRepository) Create(ctx context.Context, w *domain.Workout) error {
	_, err := r.db.Exec(ctx,
		`INSERT INTO workouts (id, user_id, started_at, notes, status)
		 VALUES ($1, $2, $3, $4, $5)`,
		w.ID, w.UserID, w.StartedAt, w.Notes, w.Status,
	)
	return err
}
...
func (r *WorkoutRepository) FindByID(ctx context.Context, id uuid.UUID) (*domain.Workout, error) {
	return r.scanWorkout(r.db.QueryRow(ctx,
		`SELECT id, user_id, started_at, ended_at, notes, status FROM workouts WHERE id = $1`, id))
}

func (r *WorkoutRepository) FindActiveForUser(ctx context.Context, userID uuid.UUID) (*domain.Workout, error) {
	return r.scanWorkout(r.db.QueryRow(ctx,
		`SELECT id, user_id, started_at, ended_at, notes, status
...
func (r *WorkoutRepository) ListForUser(ctx context.Context, userID uuid.UUID, limit int, afterStartedAt *time.Time, afterID *uuid.UUID) ([]*domain.Workout, error) {
	query := `SELECT id, user_id, started_at, ended_at, notes, status FROM workouts WHERE user_id = $1`
	args := []any{userID}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&w.ID, &w.UserID, &w.StartedAt, &w.EndedAt, &w.Notes, &w.Status); err != nil {
			return nil, err
		}
...
func (r *WorkoutRepository) scanWorkout(row pgx.Row) (*domain.Workout, error) {
	var w domain.Workout
	err := row.Scan(&w.ID, &w.UserID, &w.StartedAt, &w.EndedAt, &w.Notes, &w.Status)
