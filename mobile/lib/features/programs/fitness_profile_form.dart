...
/// The goal/experience/days-per-week/equipment/exclusions form, extracted
/// from [FitnessProfileScreen] so it can be reused standalone (via the
/// Templates tab) and embedded as the last page of the onboarding wizard.
/// Has no Scaffold/AppBar of its own — callers provide the chrome.
class FitnessProfileForm extends ConsumerStatefulWidget {
  const FitnessProfileForm({super.key, this.onSkip, this.onGenerated});

  /// When non-null, a "Skip for now" action is shown; tapping it calls this
  /// instead of doing nothing. Used by onboarding; left null for the
  /// standalone Templates-tab entry point, where there's nothing to skip.
  final VoidCallback? onSkip;

  /// Called right after a successful generateProgram(), before the existing
  /// push to ProgramReviewScreen. Used by onboarding to mark itself
  /// complete at that moment.
  final VoidCallback? onGenerated;

  @override
  ConsumerState<FitnessProfileForm> createState() => _FitnessProfileFormState();
}

class _FitnessProfileFormState extends ConsumerState<FitnessProfileForm> {
      ...
      children: [
        if (widget.onSkip != null)
          Align(
            ...
            child: TextButton(onPressed: widget.onSkip, child: const Text('Skip for now')),
          ),
